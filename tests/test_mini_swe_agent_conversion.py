...
def test_mini_swe_minimax_m3_auto_enables_thinking(tmp_path: Path):
    agent = MiniSweAgent(logs_dir=tmp_path, model_name="minimax/MiniMax-M3")

    assert (
        """-c model.model_kwargs.thinking='{"type": "enabled", "budget_tokens": 10000}'"""
        in agent._build_config_flags()
    )


def test_mini_swe_minimax_m3_user_thinking_overrides_default(tmp_path: Path):
    agent = MiniSweAgent(
        logs_dir=tmp_path,
        model_name="minimax/MiniMax-M3",
        model_kwargs={"thinking": {"type": "enabled", "budget_tokens": 500}},
    )

    assert (
        """-c model.model_kwargs.thinking='{"type": "enabled", "budget_tokens": 500}'"""
        in agent._build_config_flags()
    )
    assert "10000" not in agent._build_config_flags()


def test_mini_swe_auto_thinking_skipped_when_key_is_set_explicitly(
    tmp_path: Path,
):
    # When the caller sets `thinking` (even to None), the auto-default is
    # suppressed. `null` is forwarded to litellm, which treats it as off.
    agent = MiniSweAgent(
        logs_dir=tmp_path,
        model_name="minimax/MiniMax-M3",
        model_kwargs={"thinking": None},
    )

    cfg = agent._build_config_flags()
    assert "model.model_kwargs.thinking=null" in cfg
    assert "10000" not in cfg


def test_mini_swe_no_auto_thinking_for_other_providers(tmp_path: Path):
    openai_agent = MiniSweAgent(logs_dir=tmp_path, model_name="openai/gpt-5.5")
    claude_agent = MiniSweAgent(
        logs_dir=tmp_path, model_name="anthropic/claude-sonnet-4-5"
    )

    for cfg in (openai_agent._build_config_flags(), claude_agent._build_config_flags()):
        assert "model.model_kwargs.thinking" not in cfg


def test_mini_swe_no_auto_thinking_for_other_minimax_versions(tmp_path: Path):
    # Only the verified model gets auto-on; new versions need explicit probing
    # before being added to MiniSweAgent._AUTO_THINKING_KWARGS.
    for model_name in ("minimax/MiniMax-M2", "minimax/MiniMax-M4-hypothetical"):
        agent = MiniSweAgent(logs_dir=tmp_path, model_name=model_name)
        assert "model.model_kwargs.thinking" not in agent._build_config_flags(), (
            f"unexpected auto-on for {model_name}"
        )


def test_mini_swe_auto_thinking_kwargs_map_is_discoverable():
    # Maps are introspectable so callers/operators can audit defaults without
    # reading source.
    assert MiniSweAgent._AUTO_THINKING_KWARGS == {
        "minimax/MiniMax-M3": {
