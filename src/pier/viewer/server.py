...
def _register_job_endpoints(app: FastAPI, jobs_dir: Path) -> None:
    ...
    async def get_upload_status(job_name: str) -> dict[str, Any]:
        """Probe whether this job is already on Pier Hub.
        ...
        Returns one of:
          ...
          * ``unauthenticated`` — ``pier auth login`` hasn't run on this box.
          * ``unavailable`` — network / RPC error reaching Pier Hub.
          * ``unknown`` — unexpected error; conservative fallback.
        """
        from pier.constants import PIER_VIEWER_JOBS_URL
        from pier.models.job.result import JobResult
        from pier.upload.db_client import UploadDB
        ...
        return {
            "status": "uploaded",
            "job_id": job_id,
            "view_url": f"{PIER_VIEWER_JOBS_URL}/{job_id}",
        }
    ...
    async def upload_job(
        ...
        """
        from pier.constants import PIER_VIEWER_JOBS_URL
        from pier.upload.uploader import Uploader
        ...
        return {
            ...
            "view_url": f"{PIER_VIEWER_JOBS_URL}/{result.job_id}",
